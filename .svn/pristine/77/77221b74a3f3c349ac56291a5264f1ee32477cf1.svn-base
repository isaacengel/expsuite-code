...
[Obj.Data.IR]=shiftdim(AA_Window(shiftdim(Obj.Data.IR,2),352-96,24,12,96),1);
 waitbar(proc/numproc,h,['Calculating in progress (' num2str(proc) '/' num2str(numproc) '): Save SOFA hrtf B']); proc=proc+1;
HRTF=AA_SOFAsaveSimpleFreeFieldHRIR([TempWorkDir '\' stimPar.ID '_M_hrtf B.sofa'],Obj,meta,stimPar);

 % calc DTFs
 waitbar(proc/numproc,h,['Calculating in progress (' num2str(proc) '/' num2str(numproc) '): HRTF --> DTF']); proc=proc+1;
 HRTF.GLOBAL_Title = 'DTF B';
...
waitbar(proc/numproc,h,['Calculating in progress (' num2str(proc) '/' num2str(numproc) '): Save SOFA dtf B']); proc=proc+1;
SOFAsave([TempWorkDir '\' stimPar.ID '_M_dtf B.sofa'],Obj,1);
